...
public class PropertyGenerator : IIncrementalGenerator
{
    ...
    private static PropertyContext? GetSemanticTarget(GeneratorSyntaxContext context,
        CancellationToken cancellationToken)
    {
        ...
        if (context.SemanticModel.GetDeclaredSymbol(context.Node) is not IPropertySymbol symbol)
            ...
            return new PropertyContext
            {
                PropertyName = context.SemanticModel.GetDeclaredSymbol(context.Node)?.ToDisplayString() ??
                               "symbol is null",
                ClassName = p.Identifier.ValueText
            };
        ...
        if (attributeData == null)
            ...
            return new PropertyContext
            {
                PropertyName = "attribute data is null： " + symbol.GetAttributes().Length + " " + string.Join(", ",
                    symbol.GetAttributes().Select(a => a.AttributeClass?.ToDisplayString())),
                ClassName = p.Identifier.ValueText
            };
