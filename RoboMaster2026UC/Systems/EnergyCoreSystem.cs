    ...
    : ISystem
{
    ...
    ///     因装配成功而得到的每10秒金币奖励
    /// </summary>
    private readonly Dictionary<Camp, int> _coinIncr = new()
    {
        [Camp.Red] = 0,
        [Camp.Blue] = 0
    };
    ...
    public Task Reset(CancellationToken cancellation = new())
    {
        ...
        {
            foreach (var (c, v) in _coinIncr) economySystem.AddCoin(c, v);
        });
    ...
    {
        return energyCoreStatusCacheBox.WithReaderNamespace(new Identity(camp, 0))
            .LoadOrDefault(EnergyCoreStatusCacheKey, EnergyCoreStatus.Idle);
    }

    public IEnumerable<int> GetAvailableLevels()
    {
        if (timeSystem.Stage != JudgeSystemStage.Match) return Enumerable.Empty<int>();
    ...
    {
        if (level is < 1 or > 4)
            throw new ArgumentOutOfRangeException(nameof(level), "Level must be between 1 and 4. Got " + level);
        if (camp is not (Camp.Red or Camp.Blue))
            throw new ArgumentOutOfRangeException(nameof(camp), "Camp must be Red or Blue. Got " + camp);
        if (timeSystem.Stage != JudgeSystemStage.Match) return;
        ...
        else if (level == 2)
        {
            ...
            if (isFirst) performanceSystem.SetLevelUpperBound(camp, 7);
        }
        else if (level == 3)
        {
