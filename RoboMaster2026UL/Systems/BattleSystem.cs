...
public sealed class BattleSystem : IBattleSystem
{
    ...
    [Inject] internal RM2026ulPerformanceSystem PerformanceSystem { get; set; }
    ...
    {
        if (shooter.AmmoAllowance <= 0) return false;
    ...
    private Task CooldownTaskFor(IShooter shooter)
    {
        ...
        if (q1 > q2) BuffSystem.AddBuff(shooter.Id, RM2026ulBuffs.HeatGunLocked, 2, TimeSpan.MaxValue);

        if (q1 > q0)
        {
            ModuleSystem.SetFpvVisibilityReduced(shooter.Id, true);
            BuffSystem.AddBuff(shooter.Id, RM2026ulBuffs.HeatGunLocked, 1, TimeSpan.MaxValue);
        }
        ...
        if (MathF.Abs(q1) < 0.001f)
        {
            ...
            if (
                !BuffSystem.TryGetBuff(shooter.Id, RM2026ulBuffs.HeatGunLocked, out float v)
                // || Mathf.Approximately(v, 1)
                || MathF.Abs(v - 1) < 0.001f
            )
                BuffSystem.RemoveBuff(shooter.Id, RM2026ulBuffs.HeatGunLocked);
        }
